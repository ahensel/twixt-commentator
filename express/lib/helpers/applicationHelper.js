// Translated from application_helper.rb + main_page_helper.rb
...
// Translated from ApplicationHelper#hilite_twixt_moves
// Detects move sequences in comment text and wraps them in clickable <a> links.
function hiliteTwixtMoves(html) {
  // Regex parts (matching helper.rb exactly)
  const actionMove = '[a-xA-X](?:2[0-4]|1\\d|[1-9])\\**|swap';
  ...
  return html.replace(fullPattern, (moves) => {
    ...
    const replaced = moves.replace(nmPattern, (move) => {
      ...
      return `<a href='' class='${color}' onclick='${jumpMethod}(${firstMoveNum}, ${JSON.stringify(moveArray)}); return false;'>${move}</a>`;
    });
...
// Pagination helper — translated from main_page_helper.rb
