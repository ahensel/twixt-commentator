    ...
    const replaced = moves.replace(nmPattern, (move) => {
      ...
      if (new RegExp(actionMove).test(moveText)) {
        ...
        if (moveOffset < 0 || moveOffset > moveArray.length) {
          // Move number doesn't follow the last commentary move.
          // For non-| sequences, try to seed context from the longest
          // sequence seen so far in this comment, so e.g. "15.i9" after
          // "3.j10" in prose uses the original moves 1-14 as context.
